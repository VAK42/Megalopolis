...
class ReviewRepository {
  ...
  Future<List<Map<String, dynamic>>> getReviews(
    ...
    for (var review in reviews) {
      ...
      enrichedReviews.add({
        ...review,
        'userName': users.isNotEmpty ? users.first['name'] : 'Unknown User',
        'userAvatar': users.isNotEmpty ? users.first['avatar'] : null,
      });
    }
  ...
  Future<void> _updateTargetRating(String targetType, String targetId) async {
    ...
    final result = await db.rawQuery(
      'SELECT AVG(rating) as avgRating FROM reviews WHERE targetType = ? AND targetId = ?',
      [targetType, targetId],
    );
  ...
  Future<int> getReviewCount(String targetType, String targetId) async {
    final db = await _dbHelper.database;
    final result = await db.rawQuery(
      'SELECT COUNT(*) as count FROM reviews WHERE targetType = ? AND targetId = ?',
      [targetType, targetId],
    );
