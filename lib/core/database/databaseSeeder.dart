...
class DatabaseSeeder {
 ...
 Future<void> seed() async {
  ...
  await db.rawInsert(
   '''
  INSERT INTO promotions (id, title, subtitle, type, gradient, isActive, startDate, endDate, createdAt)
  VALUES 
  ('promo1', 'Flash Sale', 'Up To 70% OFF', 'flashSale', 'primary', 1, ?, ?, ?),
  ('promo2', 'Super Deals', 'Limited Time', 'deals', 'accent', 1, ?, ?, ?),
  ('promo3', 'New Arrivals', 'Fresh Stock', 'new', 'error', 1, ?, ?, ?)
  ''',
   [now, now + 604800000, now, now, now + 1209600000, now, now, now + 2592000000, now],
  );
  await db.rawInsert(
   '''
